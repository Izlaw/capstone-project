...
document.addEventListener('DOMContentLoaded', function () {
    ...
    conversationList.addEventListener('click', function (event) {
        ...
        if (target) {
            ...
            if (recipientId) {
                window.location.href = `/chat/${recipientId}`; // Redirect to the chat page with the recipient ID
            } else {
    ...
    window.Echo.private('chat')
        .listen('MessageSent', (event) => {
            ...
            if (existingItem) {
                ...
                if (messageParagraph) {
                    messageParagraph.textContent = messageContent || 'No content';
                } else {
                ...
                newConversationItem.innerHTML = `
                    <a href="{{ route('chat.recipient', ['recipient' => '']) }}${userId}" class="block p-4 bg-gray-100 rounded-lg shadow">
