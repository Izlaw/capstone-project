...
<body class="font-sans antialiased flex flex-col min-h-screen overflow-y-hidden">
    ...
    <div class="bg-maroonbgcolor text-lg text-center p-2 rounded mr-2 text-white border border-white">
    <p>Total Amount</p>
...
// Assuming you have the prices defined as follows
const price_xs = 200; // Example price for XS
const price_s = 250;  // Example price for S
const price_m = 300;  // Example price for M
const price_l = 350;  // Example price for L
const price_xl = 400; // Example price for XL

// Function to calculate the total amount
function calculateTotal() {
    const size_xs = parseInt(document.querySelector('input[name="size_xs"]').value) || 0;
    const size_s = parseInt(document.querySelector('input[name="size_s"]').value) || 0;
    const size_m = parseInt(document.querySelector('input[name="size_m"]').value) || 0;
    const size_l = parseInt(document.querySelector('input[name="size_l"]').value) || 0;
    const size_xl = parseInt(document.querySelector('input[name="size_xl"]').value) || 0;

    // Calculate total amount
    const totalAmount = (size_xs * price_xs) +
                        (size_s * price_s) +
                        (size_m * price_m) +
                        (size_l * price_l) +
                        (size_xl * price_xl);
...
quantityInputs.forEach(input => {
    input.addEventListener('input', calculateTotal); // Recalculate total on input change
});
